...
class PIDController:
    def __init__(self, kp, ki, kd, max_integral=10.0, min_output=-14.0, max_output=13.0, deadzone=0.2):
        self.kp = kp  # Proportional gain
        self.ki = ki  # Integral gain
        self.kd = kd  # Derivative gain
        self.max_integral = max_integral  # Maximum value for the integral term to prevent windup
        self.min_output = min_output  # Minimum output value (for negative acceleration)
        self.max_output = max_output  # Maximum output value (for positive acceleration)
        self.deadzone = deadzone  # Error deadzone for zero output
...
class PurePursuitController:
    def __init__(self, base_lookahead_distance=5.0, velocity_factor=0.5):
        self.base_lookahead_distance = base_lookahead_distance
        self.velocity_factor = velocity_factor

    def compute_steering(self, car, path):
        """
        ...
        self.lookahead_distance = self.base_lookahead_distance + car.velocity * self.velocity_factor
